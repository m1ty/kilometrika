...
`GET /api/frame.png` отдаёт карточку 800×480 в палитре Spectra 6: половинки
бега и велосипеда с последней тренировкой и итогами за неделю/месяц/год.
Эндпоинт поддерживает ETag/304 — рамка перерисовывается только при новых
данных. Подписи по умолчанию русские; `FRAME_LANG=en` переключает кадр
на английский (Week / Month / Year, km).
